...
bool LAppDelegate::Initialize(Live2DWidget*window)
{
    ...
    _window = window;   //修改成自己定义的OpenGL窗口
    if (_window == NULL)
    {
        if (DebugLogEnable)
        {
    ...
    _window->makeCurrent(); //修改为自身opengl的makeCurrent;
    glfwSwapInterval(1);
    ...
    _windowWidth = _window->width();    //获取窗口大小
...
void LAppDelegate::InitializeCubism()
{
    ...
    //not init background and others
