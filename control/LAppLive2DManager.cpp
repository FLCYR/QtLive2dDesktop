...
void LAppLive2DManager::OnTap(csmFloat32 x, csmFloat32 y)
{
    ...
    for (csmUint32 i = 0; i < _models.GetSize(); i++)
    {
        ...
        else
        {
            if (DebugLogEnable)
            {
                LAppPal::PrintLog("[APP]hit area: [%s]", "Not head and Not body");
            }
            _models[i]->SetRandomExpression();
        }
    }
...
{
    csmInt32 no = (_sceneIndex + 1) % ModelDir.size();
    ChangeScene(no);
}
...
void LAppLive2DManager::ChangeScene(Csm::csmInt32 index)
{
   ...
   // modelJsonName += ".model3.json";
    ...
    QFileInfo info(ModelDir.at(index));
    std::string modelJsonName =info.fileName().toStdString();
    std::string modelPath = (info.path()+"/").toStdString();

    //LAppPal::PrintLog("[APP]model paht: %s", modelPath.c_str());
    ...
    //force to start Idle
    _models[0]->StartRandomMotion(MotionGroupIdle, PriorityForce);

    //_models[0]->GetModelMatrix()->TranslateX(0.2f);
    //改变模型渲染的位置
   // _models[0]->GetModelMatrix()->Translate(-0.8f,0.1f);
